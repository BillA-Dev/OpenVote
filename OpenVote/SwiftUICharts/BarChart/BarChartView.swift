...
public struct BarChartView : View {
    ...
    public init(data:ChartData, title: String, legend: String? = nil, style: ChartStyle = Styles.barChartStyleOrangeLight, form: CGSize? = ChartForm.medium, dropShadow: Bool? = true, cornerImage:Image? = Image(""), valueSpecifier: String? = "%.1f", animatedToBack: Bool = false){
        self.data = data
        self.title = title
        self.legend = legend
        self.style = style
        ...
        ZStack{
            ...
            VStack(alignment: .leading){
                HStack{
                    ...
                    Spacer()
                    /* REVISIT */
                    self.cornerImage?.resizable().frame(width: 28, height: 20).aspectRatio(contentMode: .fit)
                        .imageScale(.large)
